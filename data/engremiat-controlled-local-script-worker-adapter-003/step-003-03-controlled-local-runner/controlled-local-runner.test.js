...
const timeoutPath=path.resolve(process.argv[6]);
const results=[];
...
const successExecutor=async request=>({started:true,completed:true,timed_out:false,cancelled:false,exit_code:0,stdout:"syntax ok\n",stderr:"",duration_ms:12,request});
...
check("stdout_hash_created",typeof r.result.output.stdout_sha256==="string"&&r.result.output.stdout_sha256.length===64,r.result.output.stdout_sha256);
check("stderr_hash_created",typeof r.result.output.stderr_sha256==="string"&&r.result.output.stderr_sha256.length===64,r.result.output.stderr_sha256);
check("success_integrity_valid",integrityValid(r.result),r.result.integrity_sha256);
check("shell_disabled",r.result.safety.shell===false&&r.result.safety.use_shell_execute===false,String(r.result.safety.shell));
check("network_disabled",r.result.safety.network===false,String(r.result.safety.network));
...
const bad=clone(preflight);bad.safety.shell_interpolation=true;
r=await runControlled(bad,"AUTHORIZED",successExecutor,{repository_root:process.cwd()});
check("unsafe_preflight_blocked",!r.ok&&r.code==="PREFLIGHT_INVALID",r.code);

const badExecutable=clone(preflight);badExecutable.executable="powershell";
r=await runControlled(badExecutable,"AUTHORIZED",successExecutor,{repository_root:process.cwd()});
check("unregistered_executable_blocked",!r.ok&&r.code==="EXECUTABLE_NOT_ALLOWED",r.code);

const badTimeout=clone(preflight);badTimeout.timeout_seconds=999;
r=await runControlled(badTimeout,"AUTHORIZED",successExecutor,{repository_root:process.cwd()});
check("invalid_timeout_blocked",!r.ok&&r.code==="TIMEOUT_INVALID",r.code);
